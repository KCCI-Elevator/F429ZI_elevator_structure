...
void f429_oled_ui_update(uint8_t floor, uint8_t lift_dir, uint8_t special_state, bool *call_car) {
    ...
    if (special_state == BSP_STATE_INSPECTION) oled_show_scaled_string(8, 55, "INSP", 2, 1);
    else if (special_state == BSP_STATE_MOVING) oled_show_scaled_string(8, 55, "MOVE", 2, 1);
