...
float MP_Update(MotionPlanner_t *p) {
    ...
    switch (p->state) {
       ...
       case MP_ACCEL_CONST:
            //가속도 유지
            p->acceleration = p->max_accel; 
            
            // 속도 클램핑
            if(p->velocity >= p->max_vel) {
                p->velocity = p->max_vel; 
        ...
        case MP_CRUISE:
            ...
            if(fabs(dist_to_go) <= braking_distance){
        ...
        case MP_DECEL_DEC:
            p->acceleration += p->jerk * p->dt;
            // 가속도가 다시 0으로 돌아오거나, 속도가 0에 근접했을 때 완전 정지
            if(p->acceleration >= 0.0f || p->velocity <= 0.5f) {
                p->velocity = 0.0f;
                p->acceleration = 0.0f;
                p->state = MP_DONE;
            }
